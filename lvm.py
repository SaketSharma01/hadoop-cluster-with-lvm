...
def lvmremote():
	...
	ps=getpass.getpass("Enter {ip}'s password: ")
	...
	if ch==1:
		os.system(f"sshpass {ps} ssh {ip} fdisk -l")
	elif ch==2:
		diskname=input("Enter the disk name: ")
		os.system(f"sshpass {ps} ssh {ip}pvcreate {diskname}")
	elif ch==3:
		diskname=input("Enter the disk name: ")
		os.system(f"sshpass {ps} ssh {ip} pvdisplay {diskname}")
	elif ch==4:
		...
		os.system(f"sshpass {ps} ssh {ip} vgcreate {vgname} {new}")
	elif ch==5:
		vgname=input("Enter the volume group name: ")
		os.system(f"sshpass {ps} ssh {ip} vgdisplay {vgname}")
	elif ch==6:
		lvname=input("Enter the logical volume name: ")
		vgname=input("Enter the volume group name: ")
		size=input("Enter the logical volume size: ")
		os.system(f"sshpass {ps} ssh {ip} lvcreate --size {size} --name {lvname}  {vgname}")
	elif ch==7:
		lvname=input("Enter the logical volume name: ")
		vgname=input("Enter the volume group name: ")
		os.system(f"sshpass {ps} ssh {ip} lvdisplay {vgname}/{lvname} ")
	elif ch==8:
		lvname=input("Enter the logical volume name: ")
		vgname=input("Enter the volume group name: ")
		os.system(f"sshpass {ps} ssh {ip} mkfs.ext4 /dev/{vgname}/{lvname} ")
	elif ch==9:
		folder=input("Enter the folder name: ")
		lvname=input("Enter the logical volume name: ")
		vgname=input("Enter the volume group name: ")
		os.system(f"sshpass {ps} ssh {ip} mkdir /{folder} ; mount /dev/{vgname}/{lvname} /{folder} ; df -hT")
	elif ch==10:
		lvname=input("Enter the logical volume name: ")
		vgname=input("Enter the volume group name: ")
		esize=input("Enter the extending size: ")
		os.system(f"sshpass {ps} ssh {ip} lvextend --size +{esize}  /dev/{vgname}/{lvname} ; resize2fs /dev/{vgname}/{lvname} ")
	elif ch==11:
		vgname=input("Enter the volume group name: ")
		pvname=input("Enter the new physical volume nme: ")
		os.system(f"sshpass {ps} ssh {ip} vgextend {vgname}  {pvname} ")
	elif ch==12:
		rsize=input("Enter the reduced size here: ")
		folder=input("Enter the folder name: ")
		lvname=input("Enter the logical volume name: ")
		vgname=input("Enter the volume group name: ")
		os.system(f"sshpass {ps} ssh {ip} umount /{folder} ; e2fsck -f /dev/{vgname}/{lvname} ; resize2fs /dev/{vgname}/{lvname} {rsize} ")
		os.system(f"sshpass {ps} ssh {ip} lvreduce --size {rsize} /dev/{vgname}/{lvname} ; mount /dev/{vgname}/{lvname}  /{folder} ")
	elif ch==13:
		os.system(f"sshpass {ps} ssh {ip} lvs")
	elif ch==14:
		break
  else:
